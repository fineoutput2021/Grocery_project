        ...
        <a class="btn btn-info cticket" href="<?php echo base_url() ?>dcadmin/promocode/add_promocode" role="button" style="margin-bottom:12px;"> Add promocode</a>
                                      ...
                                      <table class="table table-bordered table-hover table-striped" id="userTable">
                                          <thead>
                                              <tr>
                                                  ...
                                                  <th>Minimum Amount to Apply Promocode</th>
                                                  <th>Maximum Gift Amount</th>
                                                  <th>Promocode Expiry Date</th>
                                                  <th>Added Date</th>
                                                  <th>Added By</th>
                                                  <th>Status</th>
                                                  <th>Action</th>
                  ...
                  <?php $i=1; foreach($promocode->result() as $data) { ?>
                        <tr>
                            ...
                            <td><?php echo $data->maximum_gift_amount ?></td>
                            <td>
                              <?php $newdate2 = new DateTime($data->expiry_date);
                                   echo $newdate2->format('j F, Y'); ?> </td>

                            <td>

                            <?php $newdate = new DateTime($data->date);
                                 echo $newdate->format('j F, Y, g:i a'); ?> </td>

                            <td><?php
                              $this->db->select('*');
                            $this->db->from('tbl_team');
                            $this->db->where('id',$data->added_by);
                            $team_data = $this->db->get()->row();
                            if(!empty($team_data)){
                              echo $team_data->name;
                            }
                             ?></td>
