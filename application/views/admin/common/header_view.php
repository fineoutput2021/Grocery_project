...
<script type="text/javascript">
	$(function () {
    $('.panel-heading').on('scroll', function (e) {
        $('.panel-body').scrollLeft($('.panel-heading').scrollLeft());
    });
    $('.panel-body').on('scroll', function (e) {
        $('.panel-heading').scrollLeft($('.panel-body').scrollLeft());
    });
});
$(window).on('load', function (e) {
    $('.panel-title').width($('table').width());
    $('.box-body').width($('table').width());
});
</script>
  ...
  <body class="skin-blue">
    <div class="wrapper">
      ...
      <header class="main-header">
        <!-- Logo -->
        <a href="<?=base_url().ADMIN_URL ?>/home" class="logo"><b>
          <img src="<?=base_url() ?>assets/frontend/assets/images/icon/aulogo.png" style="width:115px;" class="img-fluid blur-up header-fruitsvibe-logo fruitArshCartLogo lazyloaded" alt="">
        </b></a>
            ...
            foreach($sidebar as $sd) {
                        ...
                        if(empty($dawwa)) {
                                ...
                                <span><? echo $sd['name'] ?></span> <? if($sd['name'] == "Notification"){?> <span class="badge" style="margin-left: 88px;margin-top:0px;margin-right:0px;background-color: blue;" >
                                  <!-- <?php if(!empty($notifications_count)){ echo $notifications_count; }else{ echo "0"; } ?>  -->
                                  0
                                </span> <?php } ?>
