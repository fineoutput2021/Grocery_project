...
if (!empty($this->session->userdata('user_id'))) {
  ...
  $this->db->select('*');
    ...
    $cart_item= $this->db->count_all_results();
...
<div class="col-md-4">
<div class="card">
<h5 class="card-header">My Cart <span class="text-secondary float-right">(<?=$cart_item;?> item)</span></h5>
...
<?if(!empty($cart_data)){
  foreach ($cart_data->result() as $c_data) {
    ...
    $this->db->where('id',$c_data->product_id);
    $data_name= $this->db->get();

    foreach ($data_name->result() as $value) {
...
<button type="submit" class="btn btn-secondary w-100" name="button">Place Order / RS. <?=$t_price;?></button>
</div>
</div>
</div>
</div>
