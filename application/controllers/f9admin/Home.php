...
function __construct()
		...
		function index(){
            ...
            $this->db->select('*');
						...
						$data['total_users'] = $this->db->count_all_results();
						...
						$data['today_users'] = $this->db->count_all_results();

						$this->db->select('*');
						$this->db->from('tbl_order1');
						$this->db->where('date',$cur_date);
						$data['today_orders'] = $this->db->count_all_results();
...
$db_final_amount = 0;
	            $this->db->select('*');
						...
						$total_amount_dsa= $this->db->get();

				foreach($total_amount_dsa->result() as $dd1) {
					$db_final_amount = $db_final_amount + $dd1->total_amount;

						}
						$data['total_amount'] = $db_final_amount;




						$db_today_final_amount = 0;
							            $this->db->select('*');
												$this->db->from('tbl_order1');
												$this->db->where('date',$cur_date);
												$this->db->where('order_status',4);

												$today_total_amount_dsa= $this->db->get();
										foreach($today_total_amount_dsa->result() as $dd1) {
											$db_today_final_amount = $db_today_final_amount + $dd1->total_amount;

												}
												$data['today_amount'] = $db_today_final_amount;
			...
			public function edit_slider_images($idd)
				{
					if (!empty($this->session->userdata('admin_data'))) {
						...
						$this->load->view('admin/common/header_view', $data);
						$this->load->view('admin/slider/edit_slider_images');
						$this->load->view('admin/common/footer_view');
					} else {
	...
	function add_slider_images()
	{
		...
		$this->load->view('admin/slider/add_slider_images');
		$this->load->view('admin/common/footer_view');
	}
...
// echo $ext = pathinfo($path, PATHINFO_EXTENSION);
						...
						$this->upload_config = array(
							...
							// 'allowed_types' => '*',
							'allowed_types' => 'webp|Webp|xlsx|csv|xls|pdf|doc|docx|txt|jpg|jpeg|png',
							'max_size'      => 25000
						);
						...
						if ($typ == 1) {
							...
							$last_id = $this->base_model->insert_table("tbl_sliders", $data_insert, 1);
						}
						...
						if ($typ == 2) {
							...
							$this->db->where('id', $idw);
							$last_id = $this->db->update('tbl_sliders', $data_insert);
						}
						if ($last_id != 0) {
							redirect("dcadmin/home/view_slider", "refresh");
						} else {
		...
		public function delete_slider_all($idd)
		{
			...
			if (!empty($this->session->userdata('admin_data'))) {
				...
				if ($this->load->get_var('position') == "Super Admin") {
					...
					$zapak = $this->db->delete('tbl_sliders', array('id' => $id));
		...
		$this->session->set_flashdata('emessage',"Sorry You can't change status of yourself");
		  ...
		  if($this->load->get_var('position')=="Super Admin"){
		         ...
		         $this->db->where('id', $id);
		        $zapak=$this->db->update('tbl_sliders', $data_update);
